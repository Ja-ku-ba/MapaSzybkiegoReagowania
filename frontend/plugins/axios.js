...
export default defineNuxtPlugin((NuxtApp) => {
    ...
    if(process.client){
        ...
        if(token){
            ...
            var z = 'Bearer ' + token.access
            var v = JSON.parse(localStorage.getItem('authTokens'))
            console.log(z === axios.defaults.headers.common['Authorization'], '\n|\n', z, '\n|\n', v.access, '\n|\n', axios.defaults.headers.common['Authorization'])
            console.log(token)
            // axios.defaults.headers.common['Authorization'] = z
        }
    }
    return {
        provide: { 
